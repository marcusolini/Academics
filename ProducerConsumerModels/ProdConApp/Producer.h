...
{
public:
     CProducer(const std::size_t quantity, const uint64_t createDelay = 0, const uint64_t addDelay = 0);
     virtual ~CProducer();
     void ConnectPipeline(CProdConQueue<DATA>* pPipeline);
     void operator()();
...
private:
     CProdConQueue<DATA>* m_pPipeline;
...
template <class DATA>
CProducer<DATA>::CProducer(const std::size_t quantity, const uint64_t createDelay, const uint64_t addDelay)
     : m_quantity(quantity), m_createDelay(createDelay), m_addDelay(addDelay)
{
     //LOG_START_TRACE();
}
...
template <class DATA>
void CProducer<DATA>::ConnectPipeline(CProdConQueue<DATA>* pPipeline)
{
     if (pPipeline)
     {
