...
{
public:
     CConsumer(CProdConQueue<DATA>& queue, size_t quantity, const size_t removeDelay = 0);
     virtual ~CConsumer();
     DATA& Remove();
     size_t Quantity();
...
private:
     ...
     size_t m_quantity = 0;
     size_t m_removeDelay = 0;  // milliseconds
};


template <class DATA>
CConsumer<DATA>::CConsumer(CProdConQueue<DATA>& queue, const size_t quantity, const size_t removeDelay)
