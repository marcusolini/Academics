...
{
public:
     CProducer(CProdConQueue<DATA>& queue, const size_t quantity, const size_t createDelay = 0, const size_t addDelay = 0);
     virtual ~CProducer();
     DATA& Create();
     void Add(DATA& data);
     size_t Quantity();
...
private:
     ...
     size_t m_quantity = 0;
     size_t m_createDelay = 0;  // milliseconds
     size_t m_addDelay = 0;  // milliseconds
     DATA m_data = 0;
};


template <class DATA>
CProducer<DATA>::CProducer(CProdConQueue<DATA>& queue, const size_t quantity, const size_t createDelay, const size_t addDelay)
